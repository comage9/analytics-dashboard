...
const TrendChart: React.FC<TrendChartProps> = ({ item, category, fromDate, toDate, data: propData, setData: propSetData }) => {
...
export const ShippingTable: React.FC<{ data: TrendDataItem[]; fromDate?: string; toDate?: string }> = ({ data }) => {
  ...
  data.forEach(row => {
    dateSet.add(row.일자.split('T')[0]);
    categorySet.add(row['분류'] || '기타');
  });
  const dates = Array.from(dateSet).sort().reverse(); // 최신일자 우선
    ...
    <div className="shipping-table-container">
      <table className="shipping-table">
        <thead>
          ...
          <tr>
            {dates.map(date => [<th key={date+"box"}>수량(박스)</th>, <th key={date+"sales"}>판매금액</th>])}
          </tr>
        </thead>
        <tbody>
          {categories.map(cat => (
